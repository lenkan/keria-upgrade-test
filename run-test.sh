...
# Start setup script
docker compose run -it --rm client src/setup.ts
...
docker compose pull
docker compose run -it --rm --entrypoint "/bin/bash -c" keria "find /usr/local/var/keri/db -mindepth 1 -maxdepth 1 -exec kli migrate run --name {} \;"
docker compose run -it --rm --entrypoint "/bin/bash -c" keria "keria sig-fix --force"

# # Start new version
docker compose up -d --force-recreate keria

# # Try to connect
docker compose run -it --rm client src/connect.ts
